...
public class RealSchedulerComponent : ImmediateSchedulerComponent, IForkableComponent<IWorkScheduler, object, ImmediateSchedulerComponent>
{
	...
	public override void EnqueueItemsAndContinueWithResults<TWorkerContext, TItem, TResult, TTag>(TWorkerContext context, ReadOnlyCollection<TItem> items, WorkerFunc<TWorkerContext, TItem, TResult, TTag> func, ContinueWithCollectionResults<TWorkerContext, TItem, TResult, TTag> continueWithResults, TTag tag)
	{
		_scheduler.EnqueueItemsAndContinueWithResults(context, items, func, continueWithResults, tag);
	}

	void IForkableComponent<IWorkScheduler, object, ImmediateSchedulerComponent>.ForkForPrimaryWrite(in ForkingData data, out IWorkScheduler writer, out object reader, out ImmediateSchedulerComponent full)
	{
		writer = (full = this);
		reader = null;
	}
