...
public abstract class ScheduledTableWriterBaseChunkedDualDeclarations<TItem, TCodeWriter> : ScheduledTableWriterBase<TItem, TCodeWriter> where TCodeWriter : ICodeStream
{
	...
	protected abstract void WriteDeclarationsHeader(SourceWritingContext context, TCodeWriter writer);

	protected abstract void WriteDeclarationsPart1(SourceWritingContext context, TCodeWriter writer, ReadOnlyCollection<TItem> allItems);

	protected abstract void WriteDeclarationsPart2(SourceWritingContext context, TCodeWriter writer, ReadOnlyCollection<TItem> allItems);
	...
	protected TableInfo Schedule(IPhaseWorkScheduler<GlobalWriteContext> scheduler, ReadOnlyCollection<TItem> items, int numberOfChunks)
	{
		...
		scheduler.EnqueueItemsAndContinueWithResults(scheduler.QueuingContext, new(int, ReadOnlyCollection<TItem>)[2]
		{
			(-1, null),
			(-2, null)
		}.Concat(items.ChunkWithNumber(numberOfChunks)).ToList().AsReadOnly(), ChunkWorker, PostProcessWorker, tag);
	...
	private TCodeWriter ChunkWorker(WorkItemData<GlobalWriteContext, (int, ReadOnlyCollection<TItem>), Tag> data)
	{
		...
		{
			using (tinyProfiler.Section(TableName, "Write Declarations - Part 1"))
			{
				WriteDeclarationsPart1(sourceWritingContext, writer, data.Tag.AllItems);
			}
		}
		else if (data.Item.Item1 == -2)
		{
			using (tinyProfiler.Section(TableName, "Write Declarations - Part 2"))
			{
				WriteDeclarationsPart2(sourceWritingContext, writer, data.Tag.AllItems);
			}
		}
		else
		{
	...
	private void PostProcessWorker(WorkItemData<GlobalWriteContext, ReadOnlyCollection<ResultData<(int, ReadOnlyCollection<TItem>), TCodeWriter>>, Tag> data)
	{
		...
		{
			WriteDeclarationsHeader(sourceWritingContext, writer);
			WriteOtherStream(writer, data.Item[0].Result);
			WriteOtherStream(writer, data.Item[1].Result);
		}
