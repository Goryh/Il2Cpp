...
internal class AddOrRemoveCanExecuteChangedMethodBodyWriter : ProjectedMethodBodyWriter
{
	...
	public AddOrRemoveCanExecuteChangedMethodBodyWriter(ReadOnlyContext context, MethodReference method, MethodDefinition helperMethod)
	...
	protected override void WriteInteropCallStatementWithinTryBlock(IGeneratedMethodCodeWriter writer, string[] localVariableNames, IRuntimeMetadataAccess metadataAccess)
	{
		...
		{
			WriteMethodCallStatementWithResult(metadataAccess, "NULL", _helperMethod, MethodCallType.Normal, writer, writer.Context.Global.Services.Naming.ForInteropReturnValue(), ManagedObjectExpression, localVariableNames[0]);
		}
		else
		{
