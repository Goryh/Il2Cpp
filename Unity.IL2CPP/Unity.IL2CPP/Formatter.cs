...
public class Formatter
{
	...
	{
		string stringValue = value.ToString("G17");
	...
	{
		string stringValue = value.ToString("G9");
	...
	public static string AsUTF8CppStringLiteral(string str)
	{
		...
		bool isInHexEscapeSequence = false;
		result.Append('"');
		byte[] array = bytes;
		for (int i = 0; i < array.Length; i++)
		{
			...
			default:
			{
				...
				handler.AppendFormatted(c.ToString("X"));
				stringBuilder.Append(ref handler);
				isInHexEscapeSequence = true;
				break;
			}
			case 32:
			case 33:
